...
for($count=0;$count<count($formFields);$count++)
{
	...
	$dataType = $Attribute->get_field_type_details($param)['data'];
	switch($field['dataType'])
	{
		...
		case 3: // number field
			...
			<div class="<?= $class ?>" style="margin-bottom:10px;">
				<div class="col-sm-3 control-label"><?= $field['fieldName'] ?></div>
		...
		case 4: // drop down field
			...
			$options = $Attribute->get_field_option_list($param)['data'];
			?>
			<div class="<?= $class ?>" style="margin-bottom:10px;" >
				<div class="col-sm-3 control-label"><?= $field['fieldName'] ?></div>
				<div class="col-sm-9" style="padding:0px;">
		...
		case 9: // dependent list field
			...
			$dependentField = $Attribute->get_category_field_details($param)['data'];
			?>
			<script>
		...
		case 5: // radio button field
			...
			<div class="<?= $class ?>" style="margin-bottom:10px;">
				...
				<div class="col-sm-9" style="padding:0px;">
						...
						{
						?>
						<input type="<?= $dataType->fieldType ?>" name="<?= $field['fieldHandler'] ?>" value="<?= $options[$countInner]['id'] ?>" /> 
						<?= $options[$countInner]['optionValue'] ?>
						&nbsp; 
						<?php 	
						}
						?>
